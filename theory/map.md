...
4. 聚簇索引和非聚簇
    1. 聚簇索引是索引的逻辑顺序和物理顺序是一样的。
   
5. Mysql怎么选择主键
    1. 尽量小：int或者bigint
        1. 在数据量很大的情况下，主键太大，每次加载进内存的数据就少。磁盘的IO就会增大。
    2. 尽量自增：
        1. 如果生成的id是无序的，那B+树需要做出很大的修改，比如split，还要上提之类的。cost太大。而如果是自增的，改动就很小，加在后面就好了。
    
    
## 分布式
### 分布式id
1. 数据库号段模式。起始值，加步长。连拿1000个，先放内存里缓存着。还有next的号段，在current剩多少的时候，就去提前从数据库拿1000个。避免懒加载的IO等待。
    1. 滴滴tinyid
2. 雪花算法。
    1. 只要保证单机的一个毫秒内，不会有重复的Id产生
    2. long型。8byte, 64bit. 41bit记时间+10bit记workerid+12bit记自增的id
        1. 百度：workerid从数据库里的自增记录来拿。每次app起，就去数据库插入一条，使用它返回的自增id作为workerid
        2. 美团：使用zookeeper的有序节点。这个是applicaiton重启后，去zookkeeper拿到的还是之前的值。（而百度的应该是每次重启都是新的不同的自增的值）。
